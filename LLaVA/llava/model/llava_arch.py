...
class LlavaMetaForCausalLM(ABC):
    ...
    def select_tokens_sads(self, visual_feature_vectors, importance_scores, k, threshold_ratio=None):
        """
        Sensitivity-Aware Diverse Selection (SADS)
        ...
        with torch.no_grad():
           ...
           cosine_sim = F.cosine_similarity(
               visual_feature_vectors.unsqueeze(1),
               visual_feature_vectors.unsqueeze(0),
               dim=-1
           )
        ...
        if importance_scores.max() > importance_scores.min():
           ...
           sens_weight = torch.ones_like(importance_scores)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features, pre_image_features = self.encode_images(concat_images)
        ...
        if 'LAYER_INDEX' in os.environ and os.environ['LAYER_INDEX']=='0':
            ...
            # =================== Proposed method [Training-free token pruning via Zeroth order sensitivity] ================
            _,  N_v, d_v = pre_image_features.shape


            original_features = pre_image_features[0]  # [N_v, d_v]

            num_refine = int(os.environ['NOISERECOV_NUM'])
            h = float(os.environ['NOISERECOV_INTENS'])


            device = original_features.device
            dtype = original_features.dtype

            N_v, d_v = original_features.shape
            m = num_refine


            # 2) m direction perturbations for each dim
            u = torch.randn(m, d_v, device=device, dtype=dtype)### noise for each dim
            u = u / (u.norm(dim=-1, keepdim=True) + 1e-12)   # normalize
            u_expanded = u.unsqueeze(1).expand(-1, N_v, -1)  # Applying the same u_j to all visual tokens

            # 3) +h, -h perturbations
            perturb_plus = original_features.unsqueeze(0) + h * u_expanded
            perturb_minus = original_features.unsqueeze(0) - h * u_expanded

            # 4) projector input flatten → [1, m*N_v, d_v]
            ...
            # 6) sensitivity to vision token (ignore text)
            delta_proj = proj_plus - proj_minus    # [m, N_v, d_l]
            coeff = delta_proj.norm(dim=-1) / (2*h)   # [m, N_v]
            importance_scores = coeff.mean(dim=0)  # [N_v], mean , var

            # 7) sensitivity to vision token (hybrid)
            selected_visual_tokens = self.select_tokens_sads(
                image_features[0],      # [N_v, d_v] image_features[0]
                importance_scores,          # [N_v]
                k=int(diverse_ratio * img_feature_len))
